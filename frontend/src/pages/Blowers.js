...
const Blowers = () => {
  ...
  useEffect(() => {
    const fetchBlowerData = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/motor-health/equipment/3', {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
        if (!response.ok) {
          throw new Error('Failed to fetch blower data');
        }
  ...
  const handleExportPDF = async () => {
    setExporting(true);
    try {
      const response = await fetch('/api/pdf-report/equipment/3', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      link.href = url;
      link.setAttribute('download', 'blower_performance_report.pdf');
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Blower Parameters Over Time',
      },
    },
  };

  if (loading) return <p>Loading blower data...</p>;
  if (error) return <p className="text-red-600">Error: {error}</p>;

  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Blowers Dashboard</h1>
        <button
