...
const Pumps = () => {
  ...
  useEffect(() => {
    const fetchPumpData = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/motor-health/equipment/1', {
          headers: {
  ...
  const handleExportPDF = async () => {
    setExporting(true);
    try {
      const response = await fetch('/api/pdf-report/equipment/1', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      ...
      link.href = url;
      link.setAttribute('download', 'pump_performance_report.pdf');
  ...
  const options = {
    ...
    plugins: {
      ...
      title: {
        display: true,
        text: 'Pump Parameters Over Time',
      },
    },
  };

  if (loading) return <p>Loading pump data...</p>;
  if (error) return <p className="text-red-600">Error: {error}</p>;

  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Pumps Dashboard</h1>
        <button
